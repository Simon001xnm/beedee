  ...
  if (variant === 'minimal') {
    return (
      <Link href={`/shop/product/${product.id}`} className={cn("group bg-white rounded-lg overflow-hidden border border-transparent hover:border-accent transition-all duration-300", className)}>
        ...
        <div className="p-3">
          <h4 className="text-[11px] font-medium line-clamp-1 mb-1 text-gray-700">{product.name}</h4>
      ...
      <div className="p-4 flex flex-col gap-2">
        <div className="flex flex-col">
          <Link href={`/shop/product/${product.id}`}>
            <h3 className="text-sm font-bold text-gray-800 line-clamp-2 hover:text-primary transition-colors min-h-[40px]">
              {product.name}
            </h3>
          </Link>
